...
function theme_enqueue_styles() {
	...
	wp_enqueue_style( 'child-style',
		...
		array('style', 'plugins'), '1.0', false
	);

	// App
	wp_register_script( 'app', get_stylesheet_directory_uri() . '/lib/js/app.js', array('jquery', 'plugins', 'googlemaps'), null, true );

	// Vektor object
	$vektor_js_object = array(
		'sitename' => get_bloginfo('name'),
		'themepath' => get_stylesheet_directory_uri(),
		'ajaxurl' => admin_url('admin-ajax.php'),
		'loadingmessage' => __('Please wait...', 'vektor'),
	);

	if(function_exists('get_field')) {
		$vektor_js_object = wp_parse_args(array(
			'sitename' => get_bloginfo('name'),
			'themepath' => get_stylesheet_directory_uri(),
			'ajaxurl' => admin_url('admin-ajax.php'),
			'loadingmessage' => __('Please wait...', 'vektor'),
			...
			'contact_zip' => get_field('contact_zip', 'option'),
			'contact_city' => get_field('contact_city', 'option'),
			'contact_location' => get_field('contact_location', 'option')
		), $vektor_js_object);
	}

	wp_localize_script( 'app', 'vektor', $vektor_js_object);
}
